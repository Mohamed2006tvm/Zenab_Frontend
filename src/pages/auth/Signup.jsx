...
// Inline Zenab ID modal (replaces window.alert)
function ZenabIdModal({ zenabId, onClose }) {
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
            <div className="w-full max-w-sm bg-[#111827] border border-emerald-500/30 rounded-2xl p-8 text-center shadow-2xl shadow-emerald-500/10">
                <div className="w-16 h-16 bg-emerald-500/10 border border-emerald-500/30 rounded-full flex items-center justify-center text-3xl mx-auto mb-5">
                    🎉
                </div>
                <h2 className="text-white text-xl font-bold mb-2">Account Created!</h2>
                <p className="text-slate-400 text-sm mb-6">
                    Your unique Analysis ID has been generated. Save it — you'll need it for your measurements.
                </p>
                <div className="bg-[#0d1528] border border-emerald-500/40 rounded-xl p-4 mb-6">
                    <p className="text-slate-500 text-xs uppercase tracking-widest mb-2">Your Zenab Analysis ID</p>
                    <p className="text-emerald-400 font-mono text-3xl font-extrabold tracking-wider">{zenabId}</p>
                </div>
                <button
                    onClick={onClose}
                    className="w-full py-2.5 bg-gradient-to-r from-emerald-500 to-cyan-500 hover:from-emerald-400 hover:to-cyan-400 text-white font-semibold rounded-lg transition-all duration-200 shadow-lg shadow-emerald-500/25"
                >
...
export default function Signup() {
    ...
    const [generatedId, setGeneratedId] = useState(null); // shows modal when set

    const generateZenabId = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const zenabId = generateZenabId();
            const nameWithId = `${form.fullName} (ID: ${zenabId})`;
            await signUp(form.email, form.password, nameWithId);
            setGeneratedId(zenabId); // show modal instead of window.alert
        } catch (err) {
    ...
    const PasswordField = ({ label, field, show, onToggle, placeholder }) => (
        <div>
    ...
    return (
        <>
            ...
            {generatedId && (
                <ZenabIdModal
                    zenabId={generatedId}
                    onClose={() => { setGeneratedId(null); navigate('/analyze'); }}
                />
            )}
                ...
                <div className="relative w-full max-w-md">
                    ...
                    <div className="bg-[#111827] border border-slate-800 rounded-2xl p-8">
                        ...
                        <div className="mt-6 flex items-center justify-between">
                            <span className="border-b border-slate-700 w-1/5 lg:w-1/4" />
